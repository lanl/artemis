...
#ifndef UTILS_FLUXES_RIEMANN_LLF_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_LLF_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::llf_xmhd, ...>
//! \brief The LLF_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::llf_xmhd, FLUID_TYPE> {
 public:
    ...
    for (int n = 0; n < nspecies; ++n) {
      ...
      parthenon::par_for_inner(
		...
		du_Jz = a_EJ * (wr_iJz - wl_iJz);
            ...
            // Compute the LLF_XMHD flux at interface
		...
		/*Real sign_frho = (frho == 0.) ? 0. : ((frho>0.) ? 1. : -1.); 
		Real Se_diffuse = (sign_frho==0.) ? 0. : (
				(sign_frho>0.) ? frho*Se_l/wl_idn :
						 frho*Se_r/wr_idn);
		q.flux(b, dir, iPe, k, j, i) = Se_diffuse;*/
              } 
            }
          });
    }
...
#endif // UTILS_FLUXES_RIEMANN_LLF_XMHD_HPP_
