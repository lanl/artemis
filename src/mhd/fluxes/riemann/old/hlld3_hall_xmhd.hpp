...
//========================================================================================
//! \file hlld3_hall_xmhd.hpp
//! \brief Local Lax Friedrichs (HLLD3_HALL_XMHD) Riemann solver, also known as Rusanov's
//! method, for hydrodynamics.
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_HLLD3_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_HLLD3_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::hlld3_hall_xmhd, ...>
//! \brief The HLLD3_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::hlld3_hall_xmhd, FLUID_TYPE> {
 public:
...
// ==========================================================================================
	    ...
	    // YH: save rho** for below as HLLD3 solver will update it as intermediate for flux
...
// ==========================================================================================
	    ...
	    // YH: SM based on electrons instead
            const Real bmag_l = 0.5*(SQR(bxi) + SQR(wl_iby) + SQR(wl_ibz));
            const Real bmag_r = 0.5*(SQR(bxi) + SQR(wr_iby) + SQR(wr_ibz));
            const Real Pe_TL = wl_iPe + bmag_l;
            const Real Pe_TR = wr_iPe + bmag_r;
            const Real sM_elec = ((sR_NI-ue_r)*ne_R*ue_r - (sL_NI-ue_l)*ne_L*ue_l +
                            (Pe_TL - Pe_TR)) / ((sR_NI-ue_r)*ne_R - (sL_NI-ue_l)*ne_L);
...
#endif // UTILS_FLUXES_RIEMANN_HLLD3_HALL_XMHD_HPP_
