...
TEST_CASE("IdealGas EOS with UnitSystem wrapper", "[eos][ideal_gas][units]") {
  ...
  ArtemisUtils::EOS eos_host = singularity::UnitSystem<singularity::IdealGas>(
      ...
      singularity::eos_units_init::LengthTimeUnitsInit(), time_code_to_physical,
      mass_code_to_physical, length_code_to_physical, temperature_code_to_physical);
  ...
  SECTION("Unit conversions with different scale factors") {
    ...
    Real temp_recovered =
        eos_scaled.TemperatureFromDensityInternalEnergy(rho_code, sie_code);
