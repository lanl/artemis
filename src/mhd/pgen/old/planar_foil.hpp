  ...
  pmb->par_for(
	...
	if (in_foil1 or in_foil2) {
	  ...
	  std::uniform_real_distribution<double> dist(-1.0,1.0);
 	  double xi = dist(rng);
	  rho *= (1.0+0.2*xi); // YH: add random perturbation
	  P = rho*(shkp.Temp*eV_to_K/T0);
	} else {
	  rho = shkp.rho_o;
	  P = rho*(shkp.Temp*eV_to_K/T0);
	}
...
inline void CurrentInnerX1(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  ...
  const double current = I0 * pow(std::sin(2.*pi*time/tau),2);
  ...
  pmb->par_for_bndry(
      ...
      KOKKOS_LAMBDA(const int &l, const int &k, const int &j, const int &i) {
	const int iref = is_z;
    	v(l, TE::F3, gas::face::bfield(0), k, j, i) = Bz;
  	});
