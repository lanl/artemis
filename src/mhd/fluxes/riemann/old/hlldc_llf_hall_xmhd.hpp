...
//========================================================================================
//! \file hlldc_llf_hall_xmhd.hpp
//! \brief Local Lax Friedrichs (HLLDC_LLF_HALL_XMHD) Riemann solver, also known as Rusanov's
//! method, for hydrodynamics.
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_HLLDC_LLF_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_HLLDC_LLF_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::hlldc_llf_hall_xmhd, ...>
//! \brief The HLLDC_LLF_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::hlldc_llf_hall_xmhd, FLUID_TYPE> {
 public:
...
// ==========================================================================================
	    ...
	    const Real Bmag_l = sqrt(bxsq + (SQR(wl_iby) + SQR(wl_ibz)));
            const Real Bmag_r = sqrt(bxsq + (SQR(wr_iby) + SQR(wr_ibz)));
	    ...
	    Real cw_L = 0.5*eta_H_L*kmax + sqrt(SQR(0.5*eta_H_L*kmax) + SQR(caL));  
	    cw_L = std::min(cw_L, c_per_v);
	    const Real cmax_L = std::max(cw_L, cfl);         
	    const Real eta_H_R = lambda_ion*sqrt(Bmag_r)/(L0*ne_r);       
	    Real cw_R = 0.5*eta_H_R*kmax + sqrt(SQR(0.5*eta_H_R*kmax) + SQR(caR));
	    cw_R = std::min(cw_R, c_per_v);
            const Real cmax_R = std::max(cw_R, cfr);
	    ...
	    // YH: save rho** for below as HLLDC_LLF solver will update it as intermediate for flux
	    ...
	    const Real du_bz = amax_2 * (wr_ibz - wl_ibz);
            Real fl_d_llf = 0.5 * (fl_d + fr_d - du_d);
            Real fl_mx_llf = 0.5 * (fl_mx + fr_mx - du_mx);
            Real fl_my_llf = 0.5 * (fl_my + fr_my - du_my);
            Real fl_mz_llf = 0.5 * (fl_mz + fr_mz - du_mz);
	    Real fl_by_llf = 0.5 * (fl_by + fr_by - du_by);
            Real fl_bz_llf = 0.5 * (fl_bz + fr_bz - du_bz);
	    Real fl_e_llf = 0.5 * (fl_e + fr_e - du_e);
	    Real fl_p_llf = 0.5 * (wl_ipr + wr_ipr);

	    Real fl_d_hlld, fl_mx_hlld, fl_my_hlld, fl_mz_hlld, fl_e_hlld;
	    Real fl_by_hlld, fl_bz_hlld, fl_p_hlld;
	    ...
	    if (spd[0] >= 0.0) {
	      ...
	      // return Fl*
	      fl_d_hlld = fl_d  + ulst_d;
	      fl_mx_hlld = fl_mx + ulst_mx;
	      fl_my_hlld = fl_my + ulst_my;
	      fl_mz_hlld = fl_mz + ulst_mz;
	      fl_e_hlld = fl_e  + ulst_e;
	      fl_by_hlld = fl_by + ulst_by;
	      fl_bz_hlld = fl_bz + ulst_bz;
	      fl_p_hlld = ptl - ulst_Bmag + KEl - KEsl + ulst_mx;//ptst - ulst_Bmag;
	    } else if (spd[3] <= 0.0) {
	      // return Fr*
	      fl_d_hlld = fr_d  + urst_d;
	      fl_mx_hlld = fr_mx + urst_mx;
	      fl_my_hlld = fr_my + urst_my;
	      fl_mz_hlld = fr_mz + urst_mz;
	      fl_e_hlld = fr_e  + urst_e;
	      fl_by_hlld = fr_by + urst_by;
	      fl_bz_hlld = fr_bz + urst_bz;
	      fl_p_hlld = ptr - urst_Bmag + KEr - KEsr + urst_mx;//ptst - urst_Bmag;
	    } else if (spd[2] >= 0.0) {
	      // return Fl**
	      fl_d_hlld = fl_d  + ulst_d + uldst_d;
	      fl_mx_hlld = fl_mx + ulst_mx + uldst_mx;
	      fl_my_hlld = fl_my + ulst_my + uldst_my;
	      fl_mz_hlld = fl_mz + ulst_mz + uldst_mz;
	      fl_e_hlld = fl_e  + ulst_e + uldst_e;
	      fl_by_hlld = fl_by + ulst_by + uldst_by;
	      fl_bz_hlld = fl_bz + ulst_bz + uldst_bz;
	      fl_p_hlld = ptl - uldst_Bmag + KEl - KEdsl + ulst_mx + uldst_mx;//ptst - uldst_Bmag;
	    } else {
	      // return Fr**
	      fl_d_hlld = fr_d + urst_d + urdst_d;
	      fl_mx_hlld = fr_mx + urst_mx + urdst_mx;
	      fl_my_hlld = fr_my + urst_my + urdst_my;
	      fl_mz_hlld = fr_mz + urst_mz + urdst_mz;
	      fl_e_hlld = fr_e + urst_e + urdst_e;
	      fl_by_hlld = fr_by + urst_by + urdst_by;
	      fl_bz_hlld = fr_bz + urst_bz + urdst_bz;
	      fl_p_hlld = ptr - urdst_Bmag + KEr - KEdsr + urst_mx + urdst_mx;//ptst - urdst_Bmag;
	    }

// ==========================================================================================

	    // Check for shock
	    const Real shock = abs(ptr-ptl)/(ptr+ptl);
	    const Real kappa = 1./std::min(wl_idn, wr_idn);//1./wroe_idn;//std::min(wl_idn, wr_idn);//1.e4;
	    const Real theta = exp(-kappa * shock);

	    // Blend flux together
	    q.flux(b, dir, IDN, k, j, i) = theta*fl_d_hlld + (1.-theta)*fl_d_llf;
	    q.flux(b, dir, ivx, k, j, i) = theta*fl_mx_hlld + (1.-theta)*fl_mx_llf;
	    q.flux(b, dir, ivy, k, j, i) = theta*fl_my_hlld + (1.-theta)*fl_my_llf;
	    q.flux(b, dir, ivz, k, j, i) = theta*fl_mz_hlld + (1.-theta)*fl_mz_llf;
	    q.flux(b, dir, iby, k, j, i) = theta*fl_by_hlld + (1.-theta)*fl_by_llf;
	    q.flux(b, dir, ibz, k, j, i) = theta*fl_bz_hlld + (1.-theta)*fl_bz_llf;
	    q.flux(b, dir, IEN, k, j, i) = theta*fl_e_hlld + (1.-theta)*fl_e_llf;
	    p.flux(b, dir, IPR, k, j, i) = theta*fl_p_hlld + (1.-theta)*fl_p_llf;
	    // Correction for pressure as Artemis separates from mx
	    ...
	    Real fl_Jx_llf = 0.5 * (fsum_Jx - du_Jx) ;
            Real fl_Jy_llf = 0.5 * (fsum_Jy - du_Jy) ;
            Real fl_Jz_llf = 0.5 * (fsum_Jz - du_Jz) ;
            Real fl_Se_llf = 0.5 * (fsum_Se - du_Se) ;

	    Real fl_Jx_hllc, fl_Jy_hllc, fl_Jz_hllc, fl_Se_hllc;
	    if (sL_NI >= 0.) {
	      fl_Jx_hllc = fl_Jx;
              fl_Jy_hllc = fl_Jy;
              fl_Jz_hllc = fl_Jz;
	      ...
	      fl_Jx_hllc = fl_Jx + sL_NI*(Jx_starL-wl_iJx);
              fl_Jy_hllc = fl_Jy + sL_NI*(Jy_starL-wl_iJy); 
              fl_Jz_hllc = fl_Jz + sL_NI*(Jz_starL-wl_iJz);
              fl_Se_hllc = fl_Se + sL_NI*(Se_starL-Se_l);
	      ...
	      const Real Se_starR = Se_r*(sR_NI-ue_r)/(sR_NI-sM_elec);
              ...
              fl_Jx_hllc = fr_Jx + sR_NI*(Jx_starR-wr_iJx);
              fl_Jy_hllc = fr_Jy + sR_NI*(Jy_starR-wr_iJy);
              fl_Jz_hllc = fr_Jz + sR_NI*(Jz_starR-wr_iJz);
              fl_Se_hllc = fr_Se + sR_NI*(Se_starR-Se_r);
	    } else {
	      fl_Jx_hllc = fr_Jx;
              fl_Jy_hllc = fr_Jy;
              fl_Jz_hllc = fr_Jz;
	    ...
	    // Blend NI-fluxes 
	    q.flux(b, dir, iJx, k, j, i) = theta*fl_Jx_hllc + (1.-theta)*fl_Jx_llf;
	    q.flux(b, dir, iJy, k, j, i) = theta*fl_Jy_hllc + (1.-theta)*fl_Jy_llf;
	    q.flux(b, dir, iJz, k, j, i) = theta*fl_Jz_hllc + (1.-theta)*fl_Jz_llf;
	    q.flux(b, dir, iPe, k, j, i) = theta*fl_Se_hllc + (1.-theta)*fl_Se_llf;


if (std::isnan(sL_NI) or std::isnan(sR_NI)) {
  std::cerr <<i<<","<<j<<","<<k<<": sL_NI="<<sL_NI<<", sR_NI="<<sR_NI<<std::endl;
  std::cerr <<"flux for ideal ="<<fl_d_hlld<<", "<<fl_mx_hlld<<", "<<fl_mz_hlld<<", "<<fl_by_hlld<<", "<<fl_bz_hlld<<", "<<fl_e_hlld<<std::endl;
  std::cerr <<"qflux="<<fl_Jx_hllc<<", "<<fl_Jy_hllc<<
	     ", "<< fl_Jz_hllc<<", "<<fl_Se_hllc<<"\n";
  std::cerr <<"fsum = "<<fsum_Jx<<", "<<fsum_Jy<<", "<<fsum_Jz<<", "<<fsum_Se<<"\n";
  std::cerr <<"du_ ="<<du_Jx<<", "<<du_Jy<<", "<<du_Jz<<", "<<du_Se<<"\n";
  std::cerr << "================================\n";		    
  PARTHENON_FAIL("HLLDC_LLF failed!!!");
}
...
#endif // UTILS_FLUXES_RIEMANN_HLLDC_LLF_HALL_XMHD_HPP_
