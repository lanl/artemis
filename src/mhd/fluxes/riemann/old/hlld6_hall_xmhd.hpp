...
//========================================================================================
//! \file hlld6_hall_xmhd.hpp
//! \brief Local Lax Friedrichs (HLLD6_HALL_XMHD) Riemann solver, also known as Rusanov's
//! method, for hydrodynamics.
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_HLLD6_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_HLLD6_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::hlld6_hall_xmhd, ...>
//! \brief The HLLD6_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::hlld6_hall_xmhd, FLUID_TYPE> {
 public:
...
// ==========================================================================================
	    ...
	    const Real Bmag_l = (bxsq + (SQR(wl_iby) + SQR(wl_ibz)));
            const Real Bmag_r = (bxsq + (SQR(wr_iby) + SQR(wr_ibz)));
	    ...
	    // YH: Modify here to account for hall wavespeed
            ...
            const Real kmax = 2.*pi/dx[dir-1];

	    // YH: Other wavespeed from Huba book (Pg. 191)
	    ne_l = Z_ion * wl_idn;
	    ne_r = Z_ion * wr_idn;
            /*const Real waveL_Hall = (J0/(e_charge*n0*char_speed))*(wl_iJx/ne_l);
            const Real waveR_Hall = (J0/(e_charge*n0*char_speed))*(wr_iJx/ne_r);
            const Real ion_cycL = (Z_ion*e_charge*B0/m_ion) * std::sqrt(Bmag_l)*t0;
            const Real ion_cycR = (Z_ion*e_charge*B0/m_ion) * std::sqrt(Bmag_r)*t0;
            const Real vW_L = kmax*SQR(caL)/ion_cycL; // whistler
            const Real vW_R = kmax*SQR(caR)/ion_cycR;*/
	    //const Real cmax_L = std::min(std::max(abs(waveL_Hall), std::max(cfl, vW_L)), 
	    //		    c_per_v);
            //const Real cmax_R = std::min(std::max(abs(waveR_Hall), std::max(cfr, vW_R)), 
	    //		    c_per_v);
	    // YH: Re try hall-hll                                      
	    const Real eta_H_L = lambda_ion*sqrt(Bmag_l)/(L0*ne_l);       
	    const Real cw_L = 0.5*eta_H_L*kmax + sqrt(SQR(0.5*eta_H_L*kmax) + SQR(caL));  
	    const Real cmax_L = std::min(std::max(cw_L, cfl), c_per_v);         
	    const Real eta_H_R = lambda_ion*sqrt(Bmag_r)/(L0*ne_r);       
	    const Real cw_R = 0.5*eta_H_R*kmax + sqrt(SQR(0.5*eta_H_R*kmax) + SQR(caR));
            const Real cmax_R = std::min(std::max(cw_R, cfr), c_per_v);

	    //===============================================================================
	    // Compute Roe-avaeraged state
            const Real isdlpdr = 1.0/(std::sqrt(wl_idn) + std::sqrt(wr_idn));
            const Real wroe_idn = std::sqrt(wl_idn)*std::sqrt(wr_idn);
	    const Real ne_roe = Z_ion*wroe_idn;
            const Real wroe_ivx = (std::sqrt(wl_idn)*wl_ivx + std::sqrt(wr_idn)*wr_ivx)*isdlpdr;
            const Real wroe_ivy = (std::sqrt(wl_idn)*wl_ivy + std::sqrt(wr_idn)*wr_ivy)*isdlpdr;
            const Real wroe_ivz = (std::sqrt(wl_idn)*wl_ivz + std::sqrt(wr_idn)*wr_ivz)*isdlpdr;
            // Note Roe average of magnetic field is different (Athena++)
            const Real wroe_ibx = (std::sqrt(wr_idn)*bxi + std::sqrt(wl_idn)*bxi)*isdlpdr;
            const Real wroe_iby = (std::sqrt(wr_idn)*wl_iby + std::sqrt(wl_idn)*wr_iby)*isdlpdr;
            const Real wroe_ibz = (std::sqrt(wr_idn)*wl_ibz + std::sqrt(wl_idn)*wr_ibz)*isdlpdr;
            const Real x = 0.5*(SQR(wl_iby-wr_iby) +
                            SQR(wl_ibz-wr_ibz))/(SQR(std::sqrt(wl_idn)+std::sqrt(wr_idn)));
	    ...
	    const Real er = wr_ipr * igm1 +
            ...
            const Real hroe = ((el + wl_ipr + pbl)/std::sqrt(wl_idn)
                            + (er + wr_ipr + pbr)/std::sqrt(wr_idn))*isdlpdr;
	    ...
	    /*const Real cA_roe = std::sqrt(Bmag_roe / wroe_idn);
            const Real ion_cyc = (Z_ion*e_charge*B0/m_ion) * std::sqrt(Bmag_roe)*t0;
            const Real vW_roe = kmax*SQR(cA_roe)/ion_cyc;// whistler*/
	    //const Real cmax_roe = std::min(std::max(a_Eind, vW_roe), c_per_v);
	    //YH: Retry hall-hll
            const Real eta_H_roe = lambda_ion*sqrt(Bmag_roe)/(L0*ne_roe);
            const Real cw_roe = 0.5*eta_H_roe*kmax + sqrt(SQR(0.5*eta_H_roe*kmax) + SQR(a_Eind));
            const Real cmax_roe = std::min(std::max(cw_roe, a_Eind), c_per_v);
	    // YH: From Eq. 12 & 13 of hall-hll	   
	    spd[0] = std::min(wl_ivx-cmax_L, wr_ivx-cmax_R);
            spd[4] = std::max(wl_ivx+cmax_L, wr_ivx+cmax_R);		
	    // YH: from Eindfelt
	    spd[0] = std::min(spd[0], wroe_ivx-cmax_roe);
            spd[4] = std::max(spd[4], wroe_ivx+cmax_roe);
	    //===============================================================================

	    const Real spd0_orig = std::min( wl_ivx-cfl, wr_ivx-cfr );
            const Real spd4_orig = std::max( wl_ivx+cfl, wr_ivx+cfr );
	    ...
	    const Real sdl_orig = spd0_orig - wl_ivx;
	    const Real sdr_orig = spd4_orig - wr_ivx;
	    ...
	    const Real spd2_orig = (sdr_orig*ur_mx - sdl_orig*ul_mx + (ptl - ptr))
                        / (sdr_orig*ur_d - sdl_orig*ul_d);
	    ...
	    const Real spd1_orig = spd2_orig - std::abs(bxi)/sqrtdl;
            const Real spd3_orig = spd2_orig + std::abs(bxi)/sqrtdr;
	    ...
	    // YH: save rho** for below as HLLD6 solver will update it as intermediate for flux
	    ...
	    //--- YH: save stars values for p* before they are converted into diffusion stab
	    Real KEl = wl_idn * wl_ivx * wl_ivx;
	    Real KEr = wr_idn * wr_ivx * wr_ivx;
	    Real KEsl = ulst_d * spd[2] * spd[2];
	    Real KEsr = urst_d * spd[2] * spd[2];
	    Real KEdsl = KEsl;
	    Real KEdsr = KEsr;
	    ...
	    if (spd[0] >= 0.0) {
	      ...
	      p.flux(b, dir, IPR, k, j, i) = ptl - ulst_Bmag + KEl - KEsl + ulst_mx;//ptst - ulst_Bmag;
	    } else if (spd[3] <= 0.0) {
	      // return Fr*
	      ...
	      p.flux(b, dir, IPR, k, j, i) = ptr - urst_Bmag + KEr - KEsr + urst_mx;;//ptst - urst_Bmag;
	    } else if (spd[2] >= 0.0) {
	      // return Fl**
	      ...
	      p.flux(b, dir, IPR, k, j, i) = ptl - uldst_Bmag + KEl - KEdsl + ulst_mx + uldst_mx; //ptst - uldst_Bmag;
	    } else {
	      // return Fr**
	      ...
	      p.flux(b, dir, IPR, k, j, i) = ptr - urdst_Bmag + KEr - KEdsr + urst_mx + urdst_mx; //ptst - urdst_Bmag;
	    }
// ==========================================================================================
	    ...
	    const Real lamb12_l = - (lambda_ion*wl_iJx)/(L0*wl_idn);
            const Real lamb12_r = - (lambda_ion*wr_iJx)/(L0*wr_idn);
            const Real const1 = J0/(e_charge*n0*char_speed);
            const Real const2 = (J0*m_ion*L0)/(e_charge*n0*char_speed*Z_ion*me*lambda_ion);
	    const Real cm_l = 1.5*wl_ivx
            ...
            const Real cp_l = 1.5*wl_ivx
                        -(lambda_ion*wl_iJx)/(L0*wl_idn)
                        -const1*wl_iJx/(2.*Z_ion*wl_idn) +
                        sqrt(pow(0.5*const1*wl_iJx/(Z_ion*wl_idn) - (lambda_ion*wl_iJx)/(L0*wl_idn),2)
                        + const2*wl_iPe/(wl_idn));
            const Real cm_r = 1.5*wr_ivx
            ...
            const Real cp_r = 1.5*wr_ivx
                        -(lambda_ion*wr_iJx)/(L0*wr_idn)
                        -const1*wr_iJx/(2.*Z_ion*wr_idn) +
                        sqrt(pow(0.5*const1*wr_iJx/(Z_ion*wr_idn) - (lambda_ion*wr_iJx)/(L0*wr_idn),2)
                        + const2*wr_iPe/(wr_idn));	 
	    const Real NImax_l = std::max(abs(lamb12_l), std::max(abs(cm_l), abs(cp_l)));
	    const Real NImax_r = std::max(abs(lamb12_r), std::max(abs(cm_r), abs(cp_r)));
	    // Eq. 12 & 13 from hall-HLL (2025)
	    //const Real sL_NI = std::min(wl_ivx - NImax_l, wr_ivx - NImax_r);
	    //const Real sR_NI = std::max(wl_ivx + NImax_l, wr_ivx + NImax_r);
	    // YH: Since no Roe-average, I use arithmetic average
	    const Real w_idn = 0.5*(wl_idn + wr_idn);
	    const Real w_ivx = 0.5*(wl_ivx + wr_ivx);
	    const Real w_iJx = 0.5*(wl_iJx + wr_iJx);
	    const Real w_iPe = 0.5*(wl_iPe + wr_iPe);
	    const Real lamb12_avg = - (lambda_ion*w_iJx)/(L0*w_idn);
	    const Real cm_avg = 1.5*w_ivx
                        -(lambda_ion*w_iJx)/(L0*w_idn)
                        -const1*w_iJx/(2.*Z_ion*w_idn) -
                        sqrt(pow(0.5*const1*w_iJx/(Z_ion*w_idn) - (lambda_ion*w_iJx)/(L0*w_idn),2)
                        + const2*w_iPe/(w_idn));
            const Real cp_avg = 1.5*w_ivx
                        -(lambda_ion*w_iJx)/(L0*w_idn)
                        -const1*w_iJx/(2.*Z_ion*w_idn) +
                        sqrt(pow(0.5*const1*w_iJx/(Z_ion*w_idn) - (lambda_ion*w_iJx)/(L0*w_idn),2)
                        + const2*w_iPe/(w_idn));
	    const Real NImax_avg = std::max(abs(lamb12_avg), std::max(abs(cm_avg), abs(cp_avg)));
	    const Real sL_NI = std::min(wl_ivx - NImax_l, w_ivx - NImax_avg);  
	    const Real sR_NI = std::max(wr_ivx + NImax_r, w_ivx + NImax_avg);
	    ...
	    // YH: Get J*K from RH condition of Se
	    const Real n1 = (sL_NI*Se_r*(sR_NI-ue_r)) / pow(ne_starR,1.-gamma);
	    const Real n2 = (sR_NI*Se_l*(sL_NI-ue_l)) / pow(ne_starL,1.-gamma);
	    const Real d1 = (Se_r*(sR_NI-ue_r)) / pow(ne_starR,1.-gamma);
	    const Real d2 = (Se_l*(sL_NI-ue_l)) / pow(ne_starL,1.-gamma);
	    const Real sM_elec = (n1 - n2) / (d1 - d2);
	    ...
	    } else if (sL_NI <= 0. <= sM_elec) {
	      const Real Se_starL = Se_l*(sL_NI-ue_l)/(sL_NI-sM_elec);
              const Real Pe_starL = Se_starL*pow(ne_starL,gamma-1.);
	      q.flux(b, dir, iJx, k, j, i) = fl_Jx + sL_NI*(Jx_HLL-wl_iJx);
              q.flux(b, dir, iJy, k, j, i) = fl_Jy + sL_NI*(Jy_HLL-wl_iJy); 
              q.flux(b, dir, iJz, k, j, i) = fl_Jz + sL_NI*(Jz_HLL-wl_iJz);
              q.flux(b, dir, iPe, k, j, i) = fl_Se + sL_NI*(Se_starL-Se_l);
	    } else if (sM_elec <= 0. <= sR_NI) {
	      const Real Se_starR = Se_r*(sR_NI-ue_r)/(sR_NI-sM_elec);
              const Real Pe_starR = Se_starR*pow(ne_starR,gamma-1.);
              q.flux(b, dir, iJx, k, j, i) = fr_Jx + sR_NI*(Jx_HLL-wr_iJx);
              q.flux(b, dir, iJy, k, j, i) = fr_Jy + sR_NI*(Jy_HLL-wr_iJy);
              q.flux(b, dir, iJz, k, j, i) = fr_Jz + sR_NI*(Jz_HLL-wr_iJz);
...
#endif // UTILS_FLUXES_RIEMANN_HLLD6_HALL_XMHD_HPP_
