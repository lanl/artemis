...
ArtemisDriver<GEOM>::ArtemisDriver(ParameterInput *pin, ApplicationInput *app_in,
  ...
  do_damping = artemis_pkg->template Param<bool>("do_damping");
...
TaskCollection ArtemisDriver<GEOM>::StepTasks() {
  ...
  for (int stage = 1; stage <= integrator->nstages; stage++) {
    ...
    for (int i = 0; i < num_partitions; i++) {
      ...
      TaskID diff_zero = none;
      if (do_diffusion && do_gas) {
        diff_zero = tl.AddTask(none, Gas::ZeroDiffusionFlux, u0.get());
      }
      ...
      TaskID gas_flx = none, dust_flx = none, visc_flx = diff_zero, therm_flx = diff_zero;
      if (do_gas) {
        if (do_viscosity) {
          visc_flx = tl.AddTask(diff_zero, Gas::ViscousFlux<GEOM>, u0.get());
        }
        if (do_conduction) {
          therm_flx = tl.AddTask(diff_zero | visc_flx, Gas::ThermalFlux<GEOM>, u0.get());
        }
      ...
      auto send_flx =
          tl.AddTask(gas_flx | dust_flx | visc_flx | therm_flx,
      ...
      // Update independent data
      auto update =
          tl.AddTask(gas_flx | dust_flx | set_flx, ArtemisUtils::ApplyUpdate<GEOM>,
                     u0.get(), u1.get(), stage, integrator.get());

      // Apply "coordinate source terms" and diffusion sources
      TaskID gas_coord_src = update, dust_coord_src = update, gas_diff_src = update;
      if (do_gas) {
        gas_coord_src = tl.AddTask(update, Gas::FluxSource, u0.get(), bdt);
        if (do_diffusion) {
          gas_diff_src = tl.AddTask(gas_coord_src | visc_flx | therm_flx,
      ...
      TaskID gravity_src = gas_coord_src | gas_diff_src | dust_coord_src;
      if (do_gravity) {
        gravity_src = tl.AddTask(gas_coord_src | dust_coord_src,
      ...
      if (do_rotating_frame) {
        rframe_src = tl.AddTask(gravity_src, RotatingFrame::RotatingFrameForce<GEOM>,
                                u0.get(), time, bdt);
      }

      // Apply damping source term
      TaskID damping_src = rframe_src;
      if (do_damping) {
        damping_src =
            tl.AddTask(rframe_src, Damping::DampingSource<GEOM>, u0.get(), time, bdt);
      }

      // Apply cooling source term
      TaskID cooling_src = damping_src;
      if (do_cooling) {
        cooling_src = tl.AddTask(damping_src, Gas::Cooling::CoolingSource<GEOM>, u0.get(),
                                 time, bdt);
      }
...
template ArtemisDriver<C::spherical>::ArtemisDriver(PI *p, AI *a, M *m, const bool r);
template TaskListStatus ArtemisDriver<C::spherical>::Step();
template void ArtemisDriver<C::spherical>::PreStepTasks();
template TaskCollection ArtemisDriver<C::spherical>::StepTasks();
template TaskCollection ArtemisDriver<C::spherical>::PostStepTasks();
