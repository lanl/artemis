...
TaskStatus SetAuxillaryFields(MeshData<Real> *md) {
  ...
  Real dflr_gas = pm->packages.Get("gas").get()->template Param<Real>("dfloor");
  Real sieflr_gas = pm->packages.Get("gas").get()->template Param<Real>("siefloor");
  Real de_switch = pm->packages.Get("gas").get()->template Param<Real>("de_switch");
  ...
  parthenon::par_for(
      ...
      KOKKOS_LAMBDA(const int &b, const int &k, const int &j, const int &i) {
        ...
        for (int n = 0; n < vmesh.GetSize(b, gas::cons::density()); ++n) {
          ...
          const Real u_trial = u_e - ke;
          u_u = (u_trial > de_switch * u_e) ? u_trial : u_u;
...
template void ConsToPrim<Coordinates::spherical>(MD *md);
...
template void PrimToCons<MBD, Coordinates::spherical>(MBD *mbd);
...
template void PrimToCons<MD, Coordinates::spherical>(MD *md);
...
template void PostInitialization<Coordinates::spherical>(MB *pmb, PI *pin);
...
template TaskStatus SetAuxillaryFields<Coordinates::spherical>(MD *md);
