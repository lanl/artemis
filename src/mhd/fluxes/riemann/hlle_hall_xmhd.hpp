...
//========================================================================================
//! \file hlle_hall_xmhd.hpp
//! \brief Harten-Lax-vanLeer (HLLE_HALL_XMHD) Riemann solver which accounts for
//!        whistler wave as fastest speed
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_HLLE_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_HLLE_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::hlle_hall_xmhd, ...>
//! \brief The HLLE_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::hlle_hall_xmhd, FLUID_TYPE> {
 public:
  ...
  solve(const EOS &eos, parthenon::team_mbr_t const &member, const int b, const int k,
    ...
    for (int n = 0; n < nspecies; ++n) {
      ...
      parthenon::par_for_inner(
	    ...
	    if (do_mhd) {
	      Bmag_l = 0.5 * (SQR(wl_ibx) + SQR(wl_iby) + SQR(wl_ibz));
	      Bmag_r = 0.5 * (SQR(wr_ibx) + SQR(wr_iby) + SQR(wr_ibz));
	      fl_mx += (Bmag_l - SQR(wl_ibx));
	      fr_mx += (Bmag_r - SQR(wr_ibx));
	      fl_my += - wl_ibx*wl_iby;
	      fr_my += - wr_ibx*wr_iby;
	      fl_mz += - wl_ibx*wl_ibz;
	      fr_mz += - wr_ibx*wr_ibz;
		...
		Real eta_H_L = lambda_ion*sqrt(2.*Bmag_l)/(L0*ne_l);   
		Real cA_L = std::sqrt(2.*Bmag_l / wl_idn);    
		Real cw_L = 0.5*eta_H_L*kmax + sqrt(SQR(0.5*eta_H_L*kmax) + SQR(cA_L));
		cL_Hall = std::min(cw_L, c_per_v);
		
		Real eta_H_R = lambda_ion*sqrt(2.*Bmag_r)/(L0*ne_r);  
		Real cA_R = std::sqrt(2.*Bmag_r / wr_idn); 
	    ...
	    const Real SR_Hall = std::max(wr_ivx+cmax_R, wroe_ivx+cmax_roe);
	    ...
	    // Compute the HLLE_HALL for ideal MHD components of flux at interface
	    if (SL_Hall > 0.) {
...
#endif // UTILS_FLUXES_RIEMANN_HLLE_HALL_XMHD_HPP_
