...
//========================================================================================
#ifndef PGEN_BLAST_MHD_HPP_
#define PGEN_BLAST_MHD_HPP_
//! \file blast_mhd.hpp
//! \brief
//!
...
struct BLAST_MHD_Params {
  Real rhol, vxl, vyl, pl, bxl, byl, bzl, pel;
  Real rhor, vxr, vyr, pr, bxr, byr, bzr, per;
  Real xdisc, rho0;
};

//----------------------------------------------------------------------------------------
//! \fn void Init_BLAST_MHD_Params
//! \brief Extracts blast_mhd parameters from ParameterInput.
inline void Init_BLAST_MHD_Params(MeshBlock *pmb, ParameterInput *pin) {
  auto &artemis_pkg = pmb->packages.Get("artemis");
  Params &params = artemis_pkg->AllParams();
  if (!(params.hasKey("blast_mhd_params"))) {
    BLAST_MHD_Params blast_mhd_params;
    blast_mhd_params.rho0 = pin->GetOrAddReal("problem", "rho0", 1.0);
    blast_mhd_params.rhol = pin->GetOrAddReal("problem", "rhol", 1.0);
    blast_mhd_params.vxl = pin->GetOrAddReal("problem", "vxl", 0.0);
    blast_mhd_params.vyl = pin->GetOrAddReal("problem", "vyl", 0.0);
    blast_mhd_params.pl = pin->GetOrAddReal("problem", "pl", 10.0);
    blast_mhd_params.pel = pin->GetOrAddReal("problem", "pel", 5.0);
    blast_mhd_params.bxl = pin->GetOrAddReal("problem", "bxl", 0.7071067812);
    blast_mhd_params.byl = pin->GetOrAddReal("problem", "byl", 0.7071067812);
    blast_mhd_params.bzl = pin->GetOrAddReal("problem", "bzl", 0.0);
    blast_mhd_params.rhor = pin->GetOrAddReal("problem", "rhor", 1.0);
    blast_mhd_params.vxr = pin->GetOrAddReal("problem", "vxr", 0.0);
    blast_mhd_params.vyr = pin->GetOrAddReal("problem", "vyr", 0.0);
    blast_mhd_params.pr = pin->GetOrAddReal("problem", "pr", 0.1);
    blast_mhd_params.per = pin->GetOrAddReal("problem", "per", 0.05);
    blast_mhd_params.bxr = pin->GetOrAddReal("problem", "bxr", 0.7071067812);
    blast_mhd_params.byr = pin->GetOrAddReal("problem", "byr", 0.7071067812);
    blast_mhd_params.bzr = pin->GetOrAddReal("problem", "bzr", 0.0);
    blast_mhd_params.xdisc = pin->GetOrAddReal("problem", "xdisc", 0.1);
    params.Add("blast_mhd_params", blast_mhd_params);
  }
}

//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator::BLAST_MHD()
//! \brief Sets initial conditions for blast_mhd problem
...
inline void ProblemGenerator(MeshBlock *pmb, ParameterInput *pin) {
  ...
  PARTHENON_REQUIRE(do_gas || do_mhd, "The blast mhd problem requires both gas & mhd!");
  PARTHENON_REQUIRE(!(do_dust), "The blast mhd problem does not permit dust hydrodynamics!");
  ...
  // BLAST_MHD parameters
  auto shkp = artemis_pkg->Param<BLAST_MHD_Params>("blast_mhd_params");

  // Setup blast_mhd state
  pmb->par_for(
      "blast_mhd", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        geometry::Coords<GEOM> coords(pco, k, j, i);
        const auto &xi = coords.GetCellCenter();
	Real radius = sqrt(SQR(xi[0]) + SQR(xi[1]));
        const bool upwind = (radius <= shkp.xdisc);
	...
	v(0, gas::prim::Pe(0), k, j, i) = Pe*shkp.rho0;
	// YH: add magnetic field -> removed after fcc work
        ...
        v(0, gas::prim::Bfield(0), k, j, i) = bx*sqrt(shkp.rho0);
        v(0, gas::prim::Bfield(1), k, j, i) = by*sqrt(shkp.rho0);
        v(0, gas::prim::Bfield(2), k, j, i) = bz*sqrt(shkp.rho0);
      });
  ...
  pmb->par_for(
	...
	Real radiusf = sqrt(SQR(xf[0]) + SQR(xf[1]));
	const bool upwind = (radiusf <= shkp.xdisc);
	// YH: add magnetic field - refer to problem gen from fine_adv
	const Real bx = upwind ? shkp.bxl : shkp.bxr;
	vmag(0, TE::F1, gas::face::bfield(0), k, j, i) = bx*sqrt(shkp.rho0);
      });
  ...
  pmb->par_for(
	...
	Real radiusf = sqrt(SQR(xf[0]) + SQR(xf[1]));
        const bool upwind = (radiusf <= shkp.xdisc);
        const Real by = upwind ? shkp.byl : shkp.byr;
        vmag(0, TE::F2, gas::face::bfield(0), k, j, i) = by*sqrt(shkp.rho0);
      });
  ...
  pmb->par_for(
	...
	Real radiusf = sqrt(SQR(xf[0]) + SQR(xf[1]));
        const bool upwind = (radiusf <= shkp.xdisc);
        const Real bz = upwind ? shkp.bzl : shkp.bzr;
        vmag(0, TE::F3, gas::face::bfield(0), k, j, i) = bz*sqrt(shkp.rho0);
      });
