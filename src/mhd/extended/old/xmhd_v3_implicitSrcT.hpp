...
TaskStatus ApplyEJSource_direcSplit(MeshData<Real> *u0, MeshData<Real> *u1, const int stage,
  ...
  parthenon::par_for(
	...
	Real Ti_dim = vTi(b, TE::CC, 0, k, j, i) * T0*eV_to_K;
        Real Te_dim = vTe(b, TE::CC, 0, k, j, i) * T0*eV_to_K;
	Real Ti = vTi(b, TE::CC, 0, k, j, i);
        Real Te = vTe(b, TE::CC, 0, k, j, i);
        Real fac = (gamma-1.)*((kB*T0*eV_to_K)/(m_ion*SQR(char_speed))) *
                (me/(m_ion+me)) ;
	Real ni_dim = v0_rho(b, TE::CC, 0, k, j, i) * n0;
        Real tau_ei_dim = ((3.*pow(kB,1.5))/(8.*sqrt(2.*pi)*pow(e_charge,4))) *
                (pow(m_ion*Te_dim+me*Ti_dim,1.5) / 
                (sqrt(me*m_ion)*pow(Z_ion,2)*ni_dim*Coulomb_log)) *
                pow(4.*pi*eps0,2);
	Real tau_ei = tau_ei_dim / t0;
	Real tmp = beta_dt*fac/tau_ei;
	Real cv_ratio = 3.;  // cv,e/cv,ion
	...
	/*if (v0_Pe(b, TE::CC, 0, k, j, i) > v0_P(b, TE::CC, 0, k, j, i)) 
	  printf("i=%d: Pe=%.6e vs P=%.6e, Te=%.6e vs Te_new=%.6e, Ti=%.6e, tauei=%.6e \n",
		i,v0_Pe(b, TE::CC, 0, k, j, i),v0_P(b, TE::CC, 0, k, j, i),
		Te,Te_new,Ti,tau_ei_dim);*/
	vTe(b, TE::CC, 0, k, j, i) = Te_new;
	v0_Pe(b, TE::CC, 0, k, j, i) = ne*Te_new;
	v0_Se(b, TE::CC, 0, k, j, i) = v0_Pe(b, TE::CC, 0, k, j, i)*pow(ne,1.-gamma);

	// YH: this is horrible
	/*vTi(b, TE::CC, 0, k, j, i) = (1./(1.+tmp*cv_ratio))*Ti + 
		((tmp*cv_ratio)/(1.+tmp*cv_ratio))*Te_new;
	Real P_new = v0_rho(b, TE::CC, 0, k, j, i)*vTi(b, TE::CC, 0, k, j, i);
	v0_Ener(b, TE::CC, 0, k, j, i) += (P_new - v0_P(b, TE::CC, 0, k, j, i))/(gamma-1.);
	v0_P(b, TE::CC, 0, k, j, i) = P_new;*/
	});
...
TaskStatus ComputeExplicitSource(MeshData<Real> *u0, const int stage,
  ...
  parthenon::par_for(
	...
	// (c) Update fcc Bfield with its source term
        v0_SrcB(b, TE::F1, 0, k, j, i) = v0_NI(b, TE::F1, 0, k, j, i);
      });
  ...
  parthenon::par_for(
	...
	v0_SrcB(b, TE::F2, 0, k, j, i) = v0_NI(b, TE::F2, 0, k, j, i);
      });
  ...
  parthenon::par_for(
	...
	v0_SrcB(b, TE::F3, 0, k, j, i) = v0_NI(b, TE::F3, 0, k, j, i);
        });
  ...
  parthenon::par_for(
	...
	v0_SrcSe(b, TE::CC, 0, k, j, i) = (gamma-1.)*pow(ne,1.-gamma)*eta*Jmag;
	// -> include energy exchange betw ion & electron source term   
	/*Real Ti_dim = vTi(b, TE::CC, 0, k, j, i) * T0*eV_to_K;         
	Real Te_dim = vTe(b, TE::CC, 0, k, j, i) * T0*eV_to_K;   
        Real dim = pow(ne, 2.-gamma) *
                (gamma-1.)*((kB*T0*eV_to_K)/(m_ion*SQR(char_speed))) *
	        (me/(m_ion+me)) * t0;
	Real ni_dim = v0_rho(b, TE::CC, 0, k, j, i) * n0;
	Real tau_ei = ((3.*pow(kB,1.5))/(8.*sqrt(2.*pi)*pow(e_charge,4))) *
		...
		(sqrt(me*m_ion)*pow(Z_ion,2)*ni_dim*Coulomb_log)) *
		pow(4.*pi*eps0,2);
	Real srcT = (dim/tau_ei) * (vTi(b, TE::CC, 0, k, j, i) - vTe(b, TE::CC, 0, k, j, i)); 
	v0_SrcSe(b, TE::CC, 0, k, j, i) += srcT;*/
