...
NBodyGravityImpl(V1 &vmesh, V2 &vg, const geometry::Coords<GEOM> &coords,
  ...
  if ((omf != 0.0) || (omb != 0)) {
    ...
    vf[1] = ex1[1] * (vback[0] + vrot[0]) + ex2[1] * (vback[1] + vrot[1]) +
            ex3[1] * (vback[2] + vrot[2]);
    vf[2] = ex1[2] * (vback[0] + vrot[0]) + ex2[2] * (vback[1] + vrot[1]) +
            ex3[2] * (vback[2] + vrot[2]);
  }
  ...
  if (do_gas) {
    for (int n = 0; n < vmesh.GetSize(b, gas::prim::density()); ++n) {
      ...
      // Drag
      if (pl.cd != 0.0) {
        pl.drag(dens, vcart, vf, dt, dmom, &dek);
      }
      ...
      lforce.myArray[4] -= dmom[0] / dt;      // X-Force due to accretion and drag
