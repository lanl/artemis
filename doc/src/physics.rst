...
|code| allows for heating due to the stellar radiation absorbed by the material.
The model assumes that the material does not irradiate in the band that it absorbs this light, and so we do not need to solve the full radiation transport equation.
Instead, we solve for the attenuation of the stellar radiation field along rays from the star to each cell in the domain. 
For simplicity, |code| assumes that the star is at the center of a spherical domain with the rays propgating only radially.
The angular resolution is set by the smallest cell extent in the azimuthal and/or polar directions.

If another radiation package is active, the energy source term generated by the irradiation is handled during the radiation matter coupling step.
Otherwise, the hydro driver adds the source term to the internal energy field along with any other source terms.

The raytracing module only works with spherical coordinates, currently.

To enable the raytracing module, set ``physics/raytrace = true``, add an opacity model for the material, and a ``<radiation/raytrace>`` block. 
An example would be:
::
   ...
   <gas/opacity/uv/absorption>
   opacity_model = constant
   kappa_a = 0.1

   <radiation/raytrace>
   band = uv
   temperature_cgs = 4000.0 # K
   radius_solar    = 2.0    # Rsolar

Note that to separate the opacity used for the stellar light from the opacity used for the gas emission, we add an extra node denoting the "band" of radiation (in this case the UV band).
In practice, any word can be used for this specification. 
