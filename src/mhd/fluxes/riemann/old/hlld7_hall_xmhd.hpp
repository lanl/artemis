...
//========================================================================================
//! \file hlld7_hall_xmhd.hpp
//! \brief Local Lax Friedrichs (HLLD7_HALL_XMHD) Riemann solver, also known as Rusanov's
//! method, for hydrodynamics.
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_HLLD7_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_HLLD7_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::hlld7_hall_xmhd, ...>
//! \brief The HLLD7_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::hlld7_hall_xmhd, FLUID_TYPE> {
 public:
...
// ==========================================================================================
	    ...
	    spd[0] = std::min(wl_ivx-cmax_L, wroe_ivx-cmax_roe);
            spd[4] = std::max(wr_ivx+cmax_R, wroe_ivx+cmax_roe);		
	    //===============================================================================
	    ...
	    // YH: save rho** for below as HLLD7 solver will update it as intermediate for flux
...
// ==========================================================================================
	    ...
	    const Real fl_Jx_llf = 0.5 * (fsum_Jx - du_Jx) ;
            const Real fl_Jy_llf = 0.5 * (fsum_Jy - du_Jy) ;
            const Real fl_Jz_llf = 0.5 * (fsum_Jz - du_Jz) ;
            const Real fl_Se_llf = 0.5 * (fsum_Se - du_Se) ;
	    ...
	    const Real lamb12_min = std::min(lamb12_l, lamb12_r);
            Real sL_NI = std::min(lamb12_min, std::min(cm_min, cp_min));
	    ...
	    const Real lamb12_max = std::max(lamb12_l, lamb12_r);
            Real sR_NI = std::max(lamb12_max, std::max(cm_max, cp_max));
	    // YH: https://arxiv.org/pdf/1803.04954 -> does not work
	    const Real fhll_Jx = (sR_NI*fl_Jx - sL_NI*fr_Jx + sL_NI*sR_NI*(wr_iJx - wl_iJx))
		    	/ (sR_NI - sL_NI);
	    const Real fhll_Jy = (sR_NI*fl_Jy - sL_NI*fr_Jy + sL_NI*sR_NI*(wr_iJy - wl_iJy))
                        / (sR_NI - sL_NI);
	    const Real fhll_Jz = (sR_NI*fl_Jz - sL_NI*fr_Jz + sL_NI*sR_NI*(wr_iJz - wl_iJz))
                        / (sR_NI - sL_NI);
	    const Real fhll_Se = (sR_NI*fl_Se - sL_NI*fr_Se + sL_NI*sR_NI*(Se_r - Se_l))
                        / (sR_NI - sL_NI);
	    ...
	    const Real Se_HLL = (sR_NI*Se_r - sL_NI*Se_l - (fr_Se - fl_Se)) / (sR_NI - sL_NI);
	    ...
	    if (sL_NI >= 0.) {
	      ...
	      // YH: I can call this HLLC since I assume J constant across CD
	      fl_Jx_hllc = fl_Jx + sL_NI*(Jx_HLL-wl_iJx);
              fl_Jy_hllc = fl_Jy + sL_NI*(Jy_HLL-wl_iJy); 
              fl_Jz_hllc = fl_Jz + sL_NI*(Jz_HLL-wl_iJz);
	      ...
	      const Real Se_starR = Se_r*(sR_NI-ue_r)/(sR_NI-sM_elec);
              ...
              fl_Jx_hllc = fr_Jx + sR_NI*(Jx_HLL-wr_iJx);
              fl_Jy_hllc = fr_Jy + sR_NI*(Jy_HLL-wr_iJy);
              fl_Jz_hllc = fr_Jz + sR_NI*(Jz_HLL-wr_iJz);
...
#endif // UTILS_FLUXES_RIEMANN_HLLD7_HALL_XMHD_HPP_
