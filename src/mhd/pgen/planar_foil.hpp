...
#include "mhd/extended/resistivity.hpp"
#include <random>
...
inline void Init_PLANAR_FOIL_Params(MeshBlock *pmb, ParameterInput *pin) {
  ...
  if (!(params.hasKey("planar_foil_params"))) {
    ...
    planar_foil_params.bscale = pin->GetOrAddReal("problem", "bscale", 1.0);
...
inline void ProblemGenerator(MeshBlock *pmb, ParameterInput *pin) {
  ...
  const Real press = shkp.rho_f*(shkp.Temp*eV_to_K/T0);
  ...
  pmb->par_for(
	...
	if (i>0 and i<ib.e)
	  if (v(0, gas::prim::density(0), k, j, i-1)==shkp.rho_f or 
		v(0, gas::prim::density(0), k, j, i+1)==shkp.rho_f) {
	    if (in_foil1) in_foil1 = false;
	    if (in_foil2) in_foil2 = false;
	  }	
	Real rho, P;
	if (in_foil1 or in_foil2) {
	  ...
	  P = press;//shkp.rho_f*(shkp.Temp*eV_to_K/T0);
	} else {
	  rho = shkp.rho_o;
	  P = press;//shkp.rho_f*(shkp.Temp*eV_to_K/T0);
	}

	const Real Pe = (Z_ion/(Z_ion+1)) * P; // as 3 electrons per Al ions
...
inline void CurrentInnerX1(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  ...
  const double s = time>tau ? 0. : std::sin(2.*pi*time/tau);
  const double current = I0 * s * s;
  const double Ly = 40.e-3;
  const double Bz_dimension = - mu0 * current / (2.*Ly); // YH: approx using biot-savart by balance units -> do I divide by 2? AI say cause -B & +B on opposite ends 
  const double Bz = shkp.bscale * Bz_dimension / B0;
...
inline void CurrentOuterX1(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  ...
  const double current = I0 * s * s;
  const double Ly = 40.e-3;
  const double Bz_dimension = mu0 * current / (2.*Ly); // YH: Do I divide by 2?
  const double Bz = shkp.bscale * Bz_dimension / B0;
  /*const double tau = shkp.tau*1.e-9/t0;
  const double s = (time<0.25*tau) ? std::sin(2.*pi*time/tau) : 1.;
  const double By_dimension = 3.14875135722 * s * s;
...
inline void ConductInnerX2(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  ...
  // Ey & Ez after J update cause need other components of J
  pmb->par_for_bndry(       
      "Ey_F2", nb, IndexDomain::inner_x2, TE::E2,  
      coarse, fine,
      KOKKOS_LAMBDA(const int &l, const int &k, const int &j, const int &i) {    
        geometry::Coords<GEOM> coords(pco, k, j, i);    
	// Interpolate
	const int km1 = k - (ndim > 2 and k > 0);
	const int im1 = i - (i > 0);
	const int jp1 = j + (ndim>1 and j < rangeE_y.e);

	const Real rho = 0.25*(v(l, gas::prim::density(0), k, j, i) +
			       v(l, gas::prim::density(0), km1, j, i) +
			       v(l, gas::prim::density(0), k, j, im1) +
			       v(l, gas::prim::density(0), km1, j, im1)); 
	const Real Pe  = 0.25*(v(l, gas::prim::Pe(0), k, j, i) +
                               v(l, gas::prim::Pe(0), km1, j, i) +
                               v(l, gas::prim::Pe(0), k, j, im1) +
                               v(l, gas::prim::Pe(0), km1, j, im1));
	const Real ux  = 0.25*(v(l, gas::prim::velocity(0), k, j, i) +
                               v(l, gas::prim::velocity(0), km1, j, i) +
                               v(l, gas::prim::velocity(0), k, j, im1) +
                               v(l, gas::prim::velocity(0), km1, j, im1));	
	const Real uz  = 0.25*(v(l, gas::prim::velocity(2), k, j, i) +
                               v(l, gas::prim::velocity(2), km1, j, i) +
                               v(l, gas::prim::velocity(2), k, j, im1) +
                               v(l, gas::prim::velocity(2), km1, j, im1));
	const Real Bx = 0.5*(v(l, TE::F1, gas::face::bfield(0), k, j, i) +
			     v(l, TE::F1, gas::face::bfield(0), km1, j, i));
	const Real Bz = 0.5*(v(l, TE::F3, gas::face::bfield(0), k, j, i) +
                             v(l, TE::F3, gas::face::bfield(0), k, j, im1));
	const Real Jx = 0.25*(v(l, TE::E1, gas::edge::J(0), k, j, i) +
			      v(l, TE::E1, gas::edge::J(0), k, jp1, i) +
			      v(l, TE::E1, gas::edge::J(0), k, j, im1) +
			      v(l, TE::E1, gas::edge::J(0), k, jp1, im1));
	const Real Jz = 0.25*(v(l, TE::E3, gas::edge::J(0), k, j, i) +
                              v(l, TE::E3, gas::edge::J(0), k, jp1, i) +
                              v(l, TE::E3, gas::edge::J(0), km1, j, i) +
                              v(l, TE::E3, gas::edge::J(0), km1, jp1, i));
	// Compute 
	const Real ne = rho*Z_ion;      
        const Real eta = (v(l, TE::E2, gas::edge::J(0), k, j, i) == 0.) ? 0. : 
		ComputeEta(eta_type, ne, Pe);	
	const Real Eideal = (-ux*Bz + uz*Bx);   
	const Real Ehall = (lambda_ion/(L0*ne))*(-Jx*Bz + Jz*Bx);  
	const Real Eeta = eta*v(l, TE::E2, gas::edge::J(0), k, j, i);
        v(l, TE::E2, gas::edge::Efield(0), k, j, i) = -Eideal + Ehall + Eeta;
        });
  pmb->par_for_bndry(
      "Ez_F3", nb, IndexDomain::inner_x2, TE::E3,
      coarse, fine,
      KOKKOS_LAMBDA(const int &l, const int &k, const int &j, const int &i) {
	geometry::Coords<GEOM> coords(pco, k, j, i);
	// Interpolate
	const int jm1 = j - (ndim > 1 and j > 0);
	const int im1 = i - (i > 0);
	const int kp1 = k + (ndim > 2 and k < rangeE_z.e);

	const Real rho = 0.25*(v(l, gas::prim::density(0), k, j, i) +
			       v(l, gas::prim::density(0), k, j, im1) +
			       v(l, gas::prim::density(0), k, jm1, i) +
			       v(l, gas::prim::density(0), k, jm1, im1)); 
	const Real Pe  = 0.25*(v(l, gas::prim::Pe(0), k, j, i) +
                               v(l, gas::prim::Pe(0), k, j, im1) +
                               v(l, gas::prim::Pe(0), k, jm1, i) +
                               v(l, gas::prim::Pe(0), k, jm1, im1));
	const Real ux  = 0.25*(v(l, gas::prim::velocity(0), k, j, i) +
                               v(l, gas::prim::velocity(0), k, j, im1) +
                               v(l, gas::prim::velocity(0), k, jm1, i) +
                               v(l, gas::prim::velocity(0), k, jm1, im1));	
	const Real uy  = 0.25*(v(l, gas::prim::velocity(1), k, j, i) +
                               v(l, gas::prim::velocity(1), k, j, im1) +
                               v(l, gas::prim::velocity(1), k, jm1, i) +
                               v(l, gas::prim::velocity(1), k, jm1, im1));
	const Real Bx = 0.5*(v(l, TE::F1, gas::face::bfield(0), k, j, i) +
			     v(l, TE::F1, gas::face::bfield(0), k, jm1, i));
	const Real By = 0.5*(v(l, TE::F2, gas::face::bfield(0), k, j, i) +
                             v(l, TE::F2, gas::face::bfield(0), k, j, im1));
	const Real Jx = 0.25*(v(l, TE::E1, gas::edge::J(0), k, j, i) +
			      v(l, TE::E1, gas::edge::J(0), kp1, j, i) +
			      v(l, TE::E1, gas::edge::J(0), k, j, im1) +
			      v(l, TE::E1, gas::edge::J(0), kp1, j, im1));
	const Real Jy = 0.25*(v(l, TE::E2, gas::edge::J(0), k, j, i) +
                              v(l, TE::E2, gas::edge::J(0), kp1, j, i) +
                              v(l, TE::E2, gas::edge::J(0), k, jm1, i) +
                              v(l, TE::E2, gas::edge::J(0), kp1, jm1, i));
	// Compute 
	const Real ne = rho*Z_ion;      
        const Real eta = (v(l, TE::E3, gas::edge::J(0), k, j, i) == 0.) ? 0. :
		ComputeEta(eta_type, ne, Pe);	
	const Real Eideal = (ux*By - uy*Bx);   
	const Real Ehall = (lambda_ion/(L0*ne))*(Jx*By - Jy*Bx);  
	const Real Eeta = eta*v(l, TE::E3, gas::edge::J(0), k, j, i);
        v(l, TE::E3, gas::edge::Efield(0), k, j, i) = - Eideal + Ehall + Eeta;
        });
...
inline void ConductOuterX2(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  ...
  // Ey & Ez after J update cause need other components of J
  pmb->par_for_bndry(       
      "Ey_F2", nb, IndexDomain::outer_x2, TE::E2,  
      coarse, fine,
      KOKKOS_LAMBDA(const int &l, const int &k, const int &j, const int &i) {    
        geometry::Coords<GEOM> coords(pco, k, j, i);    
	// Interpolate
  ...
  pmb->par_for_bndry(
      "Ez_F3", nb, IndexDomain::outer_x2, TE::E3,
      coarse, fine,
      KOKKOS_LAMBDA(const int &l, const int &k, const int &j, const int &i) {
	geometry::Coords<GEOM> coords(pco, k, j, i);
	// Interpolate
