...
TaskStatus ShearingBoxImpl(MeshData<Real> *md, const Real om0, const Real qshear,
  ...
  static auto desc_flux = parthenon::MakePackDescriptor<
      gas::cons::density>(
      ...
      KOKKOS_LAMBDA(const int &b, const int &k, const int &j, const int &i) {
        ...
        if (do_gas) {
          for (int n = 0; n < vmesh.GetSize(b, gas::prim::density()); ++n) {
            ...
            const Real &fxp = vf.flux(b,X1DIR,gas::cons::density(n),k,j,i+1); 

            const Real &fzm = vf.flux(b,X3DIR,gas::cons::density(n),k,j,i);
            const Real &fzp = vf.flux(b,X3DIR,gas::cons::density(n),k+three_d,j,i);

            //vmesh(b, gas::cons::total_energy(n), k, j, i) -= rdt*(v1*dpxc + v3*dpzc);
