...
TaskStatus ComputeExplicitSource(MeshData<Real> *u0, const int stage,
  ...
  parthenon::par_for(
	...
	Real srcT = pow(ne, 2.-gamma)*      
		(gamma-1.)*((kB*T0*eV_to_K)/(m_ion*SQR(char_speed)))*  
		(me/(m_ion+me))*t0*((8.*sqrt(2.*pi)*pow(e_charge,4))/(3.*pow(kB,1.5)))*        
		((sqrt(me*m_ion)*Z_ion*n0*Coulomb_log)/(pow(T0*eV_to_K,1.5)))*
		ne*(Ti-Te)/pow(m_ion*Te+me*Ti,1.5);  
	v0_SrcSe(b, TE::CC, 0, k, j, i) += srcT;
	//if (abs(srcT)>0.) printf("(%d,%d,%d) srcT=%.6e, n0=%.2e, T0=%.6e, Ti=%.6e, Te=%.6e, ne=%.6e, P0=%.6e, v=%.6e, src=%.6e \n",i,j,k,srcT,n0,T0,Ti,Te,ne,Press0,char_speed,v0_SrcSe(b, TE::CC, 0, k, j, i));
