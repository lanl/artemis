...
//========================================================================================
//! \file hlld_hllc_hall_xmhd.hpp
//! \brief Local Lax Friedrichs (HLLD_HLLC_HALL_XMHD) Riemann solver, also known as Rusanov's
//! method, for hydrodynamics.
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_HLLD_HLLC_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_HLLD_HLLC_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::hlld_hllc_hall_xmhd, ...>
//! \brief The HLLD_HLLC_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::hlld_hllc_hall_xmhd, FLUID_TYPE> {
 public:
...
// ==========================================================================================
	    ...
	    const Real de_l = c_light*std::sqrt(eps0*me/(ne_l*SQR(e_charge)))/L0;
	    const Real de_r = c_light*std::sqrt(eps0*me/(ne_r*SQR(e_charge)))/L0;
            const Real vW_L = kmax*SQR(caL)/(ion_cycL*(1.+SQR(kmax)*SQR(de_l))); // whistler
            const Real vW_R = kmax*SQR(caR)/(ion_cycR*(1.+SQR(kmax)*SQR(de_r)));
	    const Real cmax_L = std::max(abs(waveL_Hall), std::max(cfl, vW_L));
            const Real cmax_R = std::max(abs(waveR_Hall), std::max(cfr, vW_R));
	    ...
	    const Real de_roe = c_light*std::sqrt(eps0*me/(ne_roe*SQR(e_charge)))/L0;
            const Real vW_roe = kmax*SQR(cA_roe)/(ion_cyc*(1.+SQR(kmax)*SQR(de_roe)));// whistler
	    const Real cmax_roe = std::max(a_Eind, vW_roe);
	    ...
	    // YH: save rho** for below as HLLD_HLLC solver will update it as intermediate for flux
...
// ==========================================================================================
	    ...
	    const Real sL_NI = wl_ivx - NImax_l;
	    const Real sR_NI = wr_ivx + NImax_r;
...
if (std::isnan(sL_NI) or std::isnan(sR_NI)) {
  ...
  std::cerr << "================================\n";		    
  PARTHENON_FAIL("HLLD_HLLC failed!!!");
}
...
#endif // UTILS_FLUXES_RIEMANN_HLLD_HLLC_HALL_XMHD_HPP_
