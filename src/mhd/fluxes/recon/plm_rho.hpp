...
//========================================================================================
#ifndef UTILS_FLUXES_RECONSTRUCTION_PLM_RHO_HPP_
#define UTILS_FLUXES_RECONSTRUCTION_PLM_RHO_HPP_
...
void PLM_RHO(const Real &q_im1, const Real &q_i, const Real &q_ip1, Real &ql_ip1,
...
KOKKOS_INLINE_FUNCTION
Real fr_Scurve(const Real rho) {
  // Increasing Sigmoid function
  // rho0=0.003 is when oscillation starts to appear near slow shock 
  const Real rho0 = 0.005;
  const Real kk = 1000.;    // Chosen by taking 1/0.001 from initial condition
  const Real fr = 1./(1.+std::exp(kk*(rho0-rho)));
  return fr;
}

//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::Reconstruction<RSolver::plm_rho, X1DIR, ...>
//! \brief The piecewise linear reconstruction method in the X1 direction
template <Coordinates GEOM>
class Reconstruction<ReconstructionMethod::plm_rho, X1DIR, GEOM> {
 public:
  template <typename V>
    ...
    for (int n = q.GetLowerBound(b); n <= q.GetUpperBound(b); ++n) {
      parthenon::par_for_inner(
	    ...
	    const Real fr = fr_Scurve(q(b, 0, k, j, i));
            PLM_RHO(q(b, n, k, j, i - 1), q(b, n, k, j, i), q(b, n, k, j, i + 1),
                  ql(n, i + 1), qr(n, i), fr, TVD_type);
          });
    }
  }
};
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::Reconstruction<RSolver::plm_rho, X2DIR, ...>
//! \brief The piecewise linear reconstruction method in the X2 direction
template <Coordinates GEOM>
class Reconstruction<ReconstructionMethod::plm_rho, X2DIR, GEOM> {
 public:
  template <typename V>
  KOKKOS_INLINE_FUNCTION void apply(parthenon::team_mbr_t const &member, const int b,
                                    const int k, const int j, const int il, const int iu,
    ...
    for (int n = q.GetLowerBound(b); n <= q.GetUpperBound(b); ++n) {
      parthenon::par_for_inner(
          DEFAULT_INNER_LOOP_PATTERN, member, il, iu, [&](const int i) {
	    const Real fr = fr_Scurve(q(b, 0, k, j, i));
            PLM_RHO(q(b, n, k, j - 1, i), q(b, n, k, j, i), q(b, n, k, j + 1, i),
                ql_jp1(n, i), qr_j(n, i), fr, TVD_type);
          });
    }
  }
};
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::Reconstruction<RSolver::plm_rho, X3DIR, ...>
//! \brief The piecewise linear reconstruction method in the X3 direction
template <Coordinates GEOM>
class Reconstruction<ReconstructionMethod::plm_rho, X3DIR, GEOM> {
 public:
  template <typename V>
  KOKKOS_INLINE_FUNCTION void apply(parthenon::team_mbr_t const &member, const int b,
                                    const int k, const int j, const int il, const int iu,
    ...
    for (int n = q.GetLowerBound(b); n <= q.GetUpperBound(b); ++n) {
      parthenon::par_for_inner(
          DEFAULT_INNER_LOOP_PATTERN, member, il, iu, [&](const int i) {
	    const Real fr = fr_Scurve(q(b, 0, k, j, i));
            PLM_RHO(q(b, n, k - 1, j, i), q(b, n, k, j, i), q(b, n, k + 1, j, i),
                ql_kp1(n, i), qr_k(n, i), fr, TVD_type);
          });
    }
  }
};
...
#endif // UTILS_FLUXES_RECONSTRUCTION_PLM_RHO_HPP_
