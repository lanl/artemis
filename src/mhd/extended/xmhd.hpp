...
TaskStatus ComputeExplicitSource(MeshData<Real> *u0, const int stage,
  ...
  parthenon::par_for(
	...
	//const Real Chyp = 400.; 
	const Real Chyp = 200. * std::min(dx[0], std::min(dx[1], dx[2]));
	...
	Real D2JDz2 = 0.;
	// if (ndim>2) {}
	const Real f = 1. / (1.0 + exp((ne - 0.01) * 1000000.));	
	...
	v0_NI1_ed(b, TE::E1, 0, k, j, i) = v0_NI_ed(b, TE::E1, 0, k, j, i);// + f*eta_hyp*(dBz_dy - dBy_dz);
	});
  ...
  parthenon::par_for(
	...
	v0_NI1_ed(b, TE::E2, 0, k, j, i) = v0_NI_ed(b, TE::E2, 0, k, j, i);// + f*eta_hyp*(-dBz_dx + dBx_dz);
	});
  ...
  parthenon::par_for(
	...
	v0_NI1_ed(b, TE::E3, 0, k, j, i) = v0_NI_ed(b, TE::E3, 0, k, j, i);// + f*eta_hyp*(dBy_dx - dBx_dy);
	});
