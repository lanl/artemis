...
//========================================================================================
//! \file llf5_hall_xmhd.hpp
//! \brief Local Lax Friedrichs (LLF5_HALL_XMHD) Riemann solver, also known as Rusanov's
//! method, for hydrodynamics.
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_LLF5_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_LLF5_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::llf5_hall_xmhd, ...>
//! \brief The LLF5_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::llf5_hall_xmhd, FLUID_TYPE> {
 public:
...
// ==========================================================================================
	    ...
	    Real cA_L = std::sqrt(2.*pbl / wl_idn);      
	    ...
	    Real cA_R = std::sqrt(2.*pbr / wr_idn);   
	    ...
	    // YH: Other wavespeed from Huba book (Pg. 191)
            ...
            const Real wave_Hall = std::max(abs(waveL_Hall),abs(waveR_Hall));
            const Real ion_cycL = (Z_ion*e_charge*B0/m_ion) * Bmag_l*t0;
            const Real ion_cycR = (Z_ion*e_charge*B0/m_ion) * Bmag_r*t0;
            const Real vW_L = kmax*SQR(cA_L)/ion_cycL; // whistler
            const Real vW_R = kmax*SQR(cA_R)/ion_cycR;
            const Real vW = std::max(vW_L,vW_R);
            const Real max_Huba = std::min(std::max(vW, wave_Hall), c_per_v);

            const Real cmax = std::max(std::max(cmax_L, cmax_R), max_Huba);
            // YH: Eq. 67 of Miyoshi 2005
            spd[0] = std::min(wl_ivx, wr_ivx) - cmax;
            spd[4] = std::max(wl_ivx, wr_ivx) + cmax;
	    ...
	    // YH: save rho** for below as LLF5 solver will update it as intermediate for flux
	    ...
	    if (spd[0] >= 0.0) {
	      ...
	      fl_p_hlld = ptst - urdst_Bmag;
	    }

// ==========================================================================================

	    // Check for shock
	    const Real divV = (wr_ivx-wl_ivx)/dx[0] + (wr_ivy-wl_ivy)/dx[1] + (wr_ivz-wl_ivz)/dx[2];
	    const Real shock = std::max(abs(ptr-ptl)/(ptr+ptl), abs(divV));
	    const Real kappa = 1./std::min(wl_idn, wr_idn);//1.e4;
...
#endif // UTILS_FLUXES_RIEMANN_LLF5_HALL_XMHD_HPP_
