...
//========================================================================================
//! \file hllc3_hall_xmhd.hpp
//! \brief Harten-Lax-vanLeer (HLLC3_HALL_XMHD) Riemann solver which accounts for
//!        whistler wave as fastest speed
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_HLLC3_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_HLLC3_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::hllc3_hall_xmhd, ...>
//! \brief The HLLC3_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::hllc3_hall_xmhd, FLUID_TYPE> {
 public:
  ...
  solve(const EOS &eos, parthenon::team_mbr_t const &member, const int b, const int k,
    ...
    for (int n = 0; n < nspecies; ++n) {
      ...
      parthenon::par_for_inner(
	    ...
	    const Real hroe = ((el + wl_ipr + 0.5*Bmag_l)/sqrtdl 
			    + (er + wr_ipr + 0.5*Bmag_r)/sqrtdr)*isdlpdr;
	    ...
	    const Real SL_Hall = std::min(wl_ivx-cmax_L, wroe_ivx-cmax_roe);
	    const Real SR_Hall = std::max(wl_ivx+cmax_L, wroe_ivx+cmax_roe);
	    ...
	    // HLLC3 stuffs (from Shengtai Li (2005))
	    ...
	    // Compute the HLLC3_HALL for ideal MHD components of flux at interface
	    if (SL_Hall >= 0.) {
	    ...
	    } else if (SL_Hall <= 0. <= sM) {
	      const Real Bx_starL = wl_ibx;
	      const Real By_starL = ((SL_Hall-wl_ivx-((Bx_starL*wl_ibx)/
				    (wl_idn*(SL_Hall-wl_ivx))))*wl_iby 
			          - (Bx_starL-wl_ibx)*wl_ivy) / 
		                    (SL_Hall-sM-(SQR(Bx_starL)/(wl_idn*(SL_Hall-wl_ivx))));
	      const Real Bz_starL = ((SL_Hall-wl_ivx-((Bx_starL*wl_ibx)/ 
				    (wl_idn*(SL_Hall-wl_ivx))))*wl_ibz
                                  - (Bx_starL-wl_ibx)*wl_ivz) / 
		                    (SL_Hall-sM-(SQR(Bx_starL)/(wl_idn*(SL_Hall-wl_ivx))));
	      const Real pstarL = wl_idn*(SL_Hall-wl_ivx)*(sM-wl_ivx) + wl_ipr 
		                - SQR(wl_ibx) + SQR(Bx_starL);
              const Real mx_starL = rho_starL*sM;
              const Real my_starL = rho_starL*wl_ivy 
	      			  - (Bx_starL*By_starL-wl_ibx*wl_iby)/(SL_Hall-sM);
              const Real mz_starL = rho_starL*wl_ivz
		      		  - (Bx_starL*Bz_starL-wl_ibx*wl_ibz)/(SL_Hall-sM);
	      const Real Bu_starL = Bx_starL*sM + 
		      		    By_starL*my_starL/rho_starL +
				    Bz_starL*mz_starL/rho_starL;
	    ...
	    } else if (sM <= 0. <= SR_Hall) {
	      const Real Bx_starR = wr_ibx;
              const Real By_starR = ((SR_Hall-wr_ivx-((Bx_starR*wr_ibx)/
                                    (wr_idn*(SR_Hall-wr_ivx))))*wr_iby
                                  - (Bx_starR-wr_ibx)*wr_ivy) /
                                    (SR_Hall-sM-(SQR(Bx_starR)/(wr_idn*(SR_Hall-wr_ivx))));
              const Real Bz_starR = ((SR_Hall-wr_ivx-((Bx_starR*wr_ibx)/
                                    (wr_idn*(SR_Hall-wr_ivx))))*wr_ibz
                                  - (Bx_starR-wr_ibx)*wr_ivz) /
                                    (SR_Hall-sM-(SQR(Bx_starR)/(wr_idn*(SR_Hall-wr_ivx))));
              const Real pstarR = wr_idn*(SR_Hall-wr_ivx)*(sM-wr_ivx) + wr_ipr
                                - SQR(wr_ibx) + SQR(Bx_starR);
              const Real mx_starR = rho_starR*sM;
              const Real my_starR = rho_starR*wr_ivy
                                  - (Bx_starR*By_starR-wr_ibx*wr_iby)/(SR_Hall-sM);
              const Real mz_starR = rho_starR*wr_ivz
                                  - (Bx_starR*Bz_starR-wr_ibx*wr_ibz)/(SR_Hall-sM);
              const Real Bu_starR = Bx_starR*sM +
                                    By_starR*my_starR/rho_starR +
                                    Bz_starR*mz_starR/rho_starR;
	    ...
	    // YH: use HLLC3 for non-ideal components
		...
		q.flux(b, dir, iJx, k, j, i) = (sR_NI*fl_Jx-sL_NI*fr_Jx+sL_NI*sR_NI
						*(wr_iJx-wl_iJx))/(sR_NI-sL_NI);
                q.flux(b, dir, iJy, k, j, i) = (sR_NI*fl_Jy-sL_NI*fr_Jy+sL_NI*sR_NI
						*(wr_iJy-wl_iJy))/(sR_NI-sL_NI);
                q.flux(b, dir, iJz, k, j, i) = (sR_NI*fl_Jz-sL_NI*fr_Jz+sL_NI*sR_NI
						*(wr_iJz-wl_iJz))/(sR_NI-sL_NI);
                q.flux(b, dir, iPe, k, j, i) = (sR_NI*fl_Se-sL_NI*fr_Se+sL_NI*sR_NI
						*(Se_r-Se_l))/(sR_NI-sL_NI);
	      }*/
		...
		const Real Pe_star = (a2_R-a2_L)/(a1_L-a1_R);
	      if (sL_NI >= 0.) {
	        q.flux(b, dir, iJx, k, j, i) = fl_Jx;
                q.flux(b, dir, iJy, k, j, i) = fl_Jy;
                q.flux(b, dir, iJz, k, j, i) = fl_Jz;
...
#endif // UTILS_FLUXES_RIEMANN_HLLC3_HALL_XMHD_HPP_
