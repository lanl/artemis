...
KOKKOS_INLINE_FUNCTION void pp_limiter(const int b, const int k, const int j, const int i, 
  ...
  // (d) Limit DB by enforcing DE = D(-uxB)
  //Real DBy_ideal = std::max(0., (abs(Dv*Bx)-abs(DEz)-abs(Du*By))/abs(vx));
  Real DBy_ideal = (Dv*Bx - Du*By - DEz) / (vx+eps);
  Real DBy_NI = (DEz + Du*By - Dv*Bx - 
		 alpha*DJx*By + alpha*DJy*Bx - (alpha/ne)*Dne*(Jx*By - Jy*Bx)) 
	  	 / (-vx + alpha*Jx + eps);
  Real DBy_Hall = (DJx*By + DJy*Bx - (1./ne)*Dne*(Jx*By - Jy*Bx))/ (Jx + eps);
  Real fy = DBy_NI/(DBy_ideal+eps);
  Real gy_2 = 1./(1.+slope*abs(fy-1.)); // greater deviation from ideal mean smaller
  //if (abs(dW(iby, i)) > DBy_ideal and abs(dW(iby, i)) > 0.)
    //      dW(iby, i) *= (DBy_ideal/abs(dW(iby, i)));
  //dW(iby, i) *= gy_2;
  //Real DBz_ideal = std::max(0., (DEy+Dw*Bx-abs(Du*Bz))/abs(vx));
  Real DBz_ideal = (DEy + Dw*Bx - Du*Bz) / (vx + eps);
  Real DBz_NI = (DEy - Du*Bz + Dw*Bx + 
		 alpha*DJx*Bz - alpha*DJz*Bx - (alpha/ne)*Dne*(-Jx*Bz + Jz*Bx)) 
	  	 / (vx - alpha*Jx + eps);
  Real DBz_Hall = (DJx*Bz - DJz*Bx - (1./ne)*Dne*(-Jx*Bz + Jz*Bx)) / (-Jx + eps);
  Real fz = DBz_NI/(DBz_ideal+eps);
  Real gz_2 = 1./(1.+slope*abs(fz-1.));
  //if (abs(dW(ibz, i)) > DBz_ideal and abs(dW(ibz, i)) > 0.)
    //      dW(ibz, i) *= (DBz_ideal/abs(dW(ibz, i)));
  //dW(ibz, i) *= gz_2;
  if (i>190 and i<210 and (abs(DBy)>0. or abs(DBz)>0.) and (std::isnan(gy_2) or std::isnan(gz_2))) {
    printf("%d: vx=%.2e, alpha=%.2e, dBy=%.2e, Iy=%.2e, NIy=%.2e & dBz=%.2e, Iz=%.2e, NIz=%.2e \n",i,vx,alpha,dW(iby, i),DBy_ideal,DBy_NI,dW(ibz, i),DBz_ideal,DBz_NI);
   printf(" -> gy2=%.2e, gz2=%.2e \n",gy_2,gz_2);
  }
