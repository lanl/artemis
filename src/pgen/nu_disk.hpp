...
//========================================================================================
#ifndef PGEN_NU_DISK_HPP_
#define PGEN_NU_DISK_HPP_
//! \file nu_disk.hpp
...
// and adapted for the nu disk setup by JTL on 12/9/24.
...
ComputeDiskProfile(const struct DiskParams pgen, const parthenon::Coordinates_t &pco,
  ...
  if (!pgen.const_nu) {
    PARTHENON_FAIL("This version only works with nu viscosity.");
  }
...
inline void InitDiskParams(MeshBlock *pmb, ParameterInput *pin) {
  ...
  if (!(params.hasKey("disk_params"))) {
    ...
    if (params.Get<bool>("do_viscosity")) {
      ...
      if (vtype == "alpha") {
        PARTHENON_FAIL("Disk pgen is only compatible with constant viscosity");
