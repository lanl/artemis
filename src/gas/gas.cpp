...
std::shared_ptr<StateDescriptor> Initialize(ParameterInput *pin) {
  ...
  if (coords == Coordinates::cartesian) {
    ...
    gas->EstimateTimestepMesh = EstimateTimestepMesh<Coordinates::spherical>;
  ...
  if (refine_field != "none") {
    ...
    if (ref_grad) {
      ...
      if (coords == C::cartesian) {
        ...
        if (ref_dens) {
          gas->CheckRefinementBlock = ScalarFirstDerivative<pdens, C::spherical>;
        } else if (ref_pres) {
          gas->CheckRefinementBlock = ScalarFirstDerivative<ppres, C::spherical>;
        }
        // Cylindrical
...
// //----------------------------------------------------------------------------------------
// //! \fn  TaskCollection Gas::OperatorSplitExample
// //! \brief Example operator split task collection
// template <Coordinates GEOM>
// TaskCollection OperatorSplitExample(Mesh *pmesh, parthenon::SimTime &tm, const Real dt)
// {
//   TaskCollection tc;
//   TaskID none(0);
...
//     auto print_msg = tl.AddTask(none, PrintMessage<GEOM>, base.get());
//   }
...
template TaskStatus ViscousFlux<Coordinates::spherical>(MeshData<Real> *md);
...
template TaskStatus ThermalFlux<Coordinates::spherical>(MeshData<Real> *md);
...
template TaskStatus DiffusionUpdate<Coordinates::cartesian>(MeshData<Real> *md,
                                                            const Real dt);
template TaskStatus DiffusionUpdate<Coordinates::spherical>(MeshData<Real> *md,
                                                            const Real dt);
...
// template TaskStatus PrintMessage<Coordinates::cartesian>(MeshData<Real> *md);
// template TaskStatus PrintMessage<Coordinates::spherical>(MeshData<Real> *md);
// template TaskStatus PrintMessage<Coordinates::cylindrical>(MeshData<Real> *md);
// template TaskStatus PrintMessage<Coordinates::axisymmetric>(MeshData<Real> *md);

// template TaskCollection OperatorSplitExample<Coordinates::cartesian>(Mesh *pm,
// 								     parthenon::SimTime
// &tm,
//                                                                      const Real dt);
// template TaskCollection OperatorSplitExample<Coordinates::spherical>(Mesh *pm,
// 								     parthenon::SimTime
// &tm,
//                                                                      const Real dt);
// template TaskCollection OperatorSplitExample<Coordinates::cylindrical>(Mesh *pm,
// 								       parthenon::SimTime
// &tm,
//                                                                        const Real dt);
// template TaskCollection OperatorSplitExample<Coordinates::axisymmetric>(Mesh *pm,
// 									parthenon::SimTime
// &tm,
//                                                                         const Real dt);
