...
//========================================================================================
//! \file llf3_hall_xmhd.hpp
//! \brief Local Lax Friedrichs (LLF3_HALL) Riemann solver, also known as Rusanov's
//! method, for hydrodynamics.
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_LLF3_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_LLF3_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::llf3_hall_xmhd, ...>
//! \brief The LLF3_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::llf3_hall_xmhd, FLUID_TYPE> {
 public:
  ...
  solve(const EOS &eos, parthenon::team_mbr_t const &member, const int b, const int k,
    ...
    for (int n = 0; n < nspecies; ++n) {
      ...
      parthenon::par_for_inner(
	    ...
	    [[maybe_unused]] Real wroe_ivx = Null<Real>();
	    [[maybe_unused]] Real cw_L = Null<Real>();
	    [[maybe_unused]] Real cw_R = Null<Real>();
	    [[maybe_unused]] Real cw_roe = Null<Real>();
	    [[maybe_unused]] Real cp_l = Null<Real>();
            [[maybe_unused]] Real cp_r = Null<Real>();
	    [[maybe_unused]] Real cm_l = Null<Real>();
            [[maybe_unused]] Real cm_r = Null<Real>();
	    [[maybe_unused]] Real lamb12_l = Null<Real>();
            [[maybe_unused]] Real lamb12_r = Null<Real>();
            if constexpr (FLUID_TYPE == Fluid::gas) {
              du_e = a * (er - el);
	      if (do_mhd) {
		...
		lamb12_l = wl_ivx - (lambda_ion*wl_iJx)/(L0*wl_idn);
		lamb12_r = wr_ivx - (lambda_ion*wr_iJx)/(L0*wr_idn);
		...
		Real eta_H_L = lambda_ion*sqrt(Bmag_l)/(L0*ne_l);
                ...
                cw_L = 0.5*eta_H_L*kmax + sqrt(SQR(0.5*eta_H_L*kmax) + SQR(cA_L));
		cw_L = std::min(cw_L, c_per_v);

                Real eta_H_R = lambda_ion*sqrt(Bmag_r)/(L0*ne_r);
                Real cA_R = std::sqrt(Bmag_r / wr_idn);
                cw_R = 0.5*eta_H_R*kmax + sqrt(SQR(0.5*eta_H_R*kmax) + SQR(cA_R));
		...
		wroe_ivx = (sqrtdl*wl_ivx + sqrtdr*wr_ivx)*isdlpdr;
		...
		const Real eta_H_roe = lambda_ion*sqrt(Bmag_roe)/(L0*wroe_idn);
                ...
                cw_roe = 0.5*eta_H_roe*kmax + sqrt(SQR(0.5*eta_H_roe*kmax) + SQR(cA_roe));
		...
		Real amax_2 = std::max(a, a_cw);
            ...
            // Compute the LLF3_HALL_XMHD flux at interface
		...
		const Real cL_Hall = std::min(cw_L, c_per_v);
		const Real cR_Hall = std::min(cw_R, c_per_v);
		const Real cL_ideal = qa;
		const Real cR_ideal = qb;
		const Real cmax_L = std::max(cL_ideal, cL_Hall);
                const Real cmax_R = std::max(cR_ideal, cR_Hall);
		const Real SL_Hall = std::min(wl_ivx-cmax_L, wroe_ivx-cw_roe);
      	        const Real SR_Hall = std::max(wr_ivx+cmax_R, wroe_ivx+cw_roe);

		const Real ptl = wl_ipr + 0.5*(SQR(wl_ibx) + SQR(wl_iby) + SQR(wl_ibz));
                const Real ptr = wr_ipr + 0.5*(SQR(wr_ibx) + SQR(wr_iby) + SQR(wr_ibz));
                const Real sM = (wr_idn*wr_ivx*(SR_Hall-wr_ivx)
                                - wl_idn*wl_ivx*(SL_Hall-wl_ivx)
                                + ptl - ptr - SQR(wl_ibx) + SQR(wr_ibx)) /
		...
		const Real rho_starL = wl_idn*(SL_Hall-wl_ivx)/(SL_Hall-sM);
	    	...
	    	if (sL_NI >= 0.) {
	      	  q.flux(b, dir, iJx, k, j, i) = fl_Jx;
	       	  ...
	       	  const Real Se_starL = Se_l*(sL_NI-ue_l)/(sL_NI-sM);
	      	  ...
	      	  const Real Se_starR = Se_r*(sR_NI-ue_r)/(sR_NI-sM);
...
#endif // UTILS_FLUXES_RIEMANN_LLF3_HALL_XMHD_HPP_
