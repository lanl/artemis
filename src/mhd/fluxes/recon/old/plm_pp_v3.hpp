...
KOKKOS_INLINE_FUNCTION void pp_limiter(const int b, const int k, const int j, const int i, 
  ...
  Drho = dW(idn, i);
  const Real Dne = Z_ion * Drho;
  // (c)i. Limit Dp: must be absolute here to be symmetric!!!
  ...
  //     D\rho too!
  const Real absDu = dx/(dt*(1.+gamma));
  ...
  const Real LdW = (rhoc*rho/rhostar)*Deltau_mag + DeltaB_mag +
		   ...
		   (Pe + 2*DeltaPe)/(gamma-1.);
  const Real rhoe = std::max(0.,
		  //(p - (dt/dx)*(abs(vx*Dp) + gamma*p*abs(Du)))/(gamma-1.));
	         (p + (dt/dx)*(neg_comp(vx*Dp) + gamma*p*neg_comp(Du))) 
	  	   / (gamma-1.));
  //const Real fdW = std::pow(rhoe / std::max(LdW, rhoe), 2.0);
  const Real fdW = std::sqrt(rhoe/std::max(LdW, rhoe));
  //const Real fdW = (rhoe==0.) ? 0. : std::exp(1.-std::max(LdW, rhoe)/rhoe);
  //const Real fdW = rhoe / std::max(LdW, rhoe);

  /*if (abs(DPe)>1.e-6 or fdW<0.9) {
...
Real f_sqrt = std::sqrt(rhoe/std::max(LdW,rhoe));
