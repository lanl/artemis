...
TaskStatus PointMassGravity(MeshData<Real> *md, const Real time, const Real dt) {
  ...
  const Real pos[3] = {gravity_pkg->template Param<Real>("x"),
                       gravity_pkg->template Param<Real>("y"),
                       gravity_pkg->template Param<Real>("z")};
  const Real rsft2 = SQR(gravity_pkg->template Param<Real>("soft"));
  ...
  parthenon::par_for(
      ...
      KOKKOS_LAMBDA(const int &b, const int &k, const int &j, const int &i) {
        ...
        Real dx[3] = {Null<Real>()};
        coords.GetCellCenter(dx);

        Real dxc[3] = {Null<Real>()};
        Real ex1[3] = {Null<Real>()};
        Real ex2[3] = {Null<Real>()};
        Real ex3[3] = {Null<Real>()};
        coords.ConvertToCartWithVec(dx, dxc, ex1, ex2, ex3);
        ...
        Real dxs[3] = {Null<Real>()};
        geometry::Coords<Coordinates::cartesian> coords_c;
        coords_c.ConvertToSph(dxc, dxs);

        // Compute acceleration
        const Real rad2 = SQR(dxs[0]) + rsft2;
...
template TaskStatus PointMassGravity<C::spherical>(MD *m, const Real t, const Real d);
