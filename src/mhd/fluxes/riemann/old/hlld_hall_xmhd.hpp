...
//========================================================================================
//! \file hlld_hall_xmhd.hpp
//! \brief Local Lax Friedrichs (HLLD_HALL_XMHD) Riemann solver, also known as Rusanov's
//! method, for hydrodynamics.
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
#ifndef UTILS_FLUXES_RIEMANN_HLLD_HALL_XMHD_HPP_
#define UTILS_FLUXES_RIEMANN_HLLD_HALL_XMHD_HPP_
...
//----------------------------------------------------------------------------------------
//! \class ArtemisUtils::RiemannSolver<RSolver::hlld_hall_xmhd, ...>
//! \brief The HLLD_HALL_XMHD Riemann solver for ideal gas hydrodynamics
template <Fluid FLUID_TYPE>
class RiemannSolver<RSolver::hlld_hall_xmhd, FLUID_TYPE> {
 public:
...
// ==========================================================================================
	    ...
	    // Compute max wave speed in L/R states (see Toro eq. 10.43)
            ...
            const Real caL = std::sqrt((SQR(bxi) + SQR(wl_iby) + SQR(wl_ibz)) / wl_idn);
            const Real caR = std::sqrt((SQR(bxi) + SQR(wr_iby) + SQR(wr_ibz)) / wr_idn);
	    ...
	    // YH: Modify here to account for hall wavespeed
            ...
            const Real kmax = 2.*pi/dx[dir-1]; // YH: Page 191 of Huba book use 2*pi/dx

	    // YH: mkae more diffusive using NI wavespeeds
	    ...
	    // YH: save rho** for below as HLLD solver will update it as intermediate for flux
	    ...
	    // YH: Should only be placed here when modifying fastest wave speed with Hall
	    //spd[0] = std::min( wl_ivx-cmax_L, wr_ivx-cmax_R );
            //spd[4] = std::max( wl_ivx+cmax_L, wr_ivx+cmax_R );
...
#endif // UTILS_FLUXES_RIEMANN_HLLD_HALL_XMHD_HPP_
