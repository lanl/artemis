...
inline void ProblemGenerator(MeshBlock *pmb, ParameterInput *pin) {
  ...
  if (do_dust) {
    ...
    PARTHENON_REQUIRE(dust_pkg->Param<int>("nspecies") == 1,
                      "Constant pgen requires a single dust species.")
  ...
  constant_params.input_system = geometry::CoordSelect(input_system);
  constant_params.system = geometry::CoordSelect(system);
  ...
  pmb->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        if (do_dust) {
          v(0, dust::prim::density(0), k, j, i) = pars.d_rho;
