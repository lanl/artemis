...
//========================================================================================
#ifndef PGEN_WHISTLER_WAVE_HPP_
#define PGEN_WHISTLER_WAVE_HPP_
//! \file whistler_wave.hpp
//! \brief
//!
...
namespace whistler_wave {

struct WHISTLER_WAVE_Params {
  Real rho0, b0, dB, beta, mode, Lx, delta_x;
};

//----------------------------------------------------------------------------------------
//! \fn void Init_WHISTLER_WAVE_Params
//! \brief Extracts whistler_wave parameters from ParameterInput.
inline void Init_WHISTLER_WAVE_Params(MeshBlock *pmb, ParameterInput *pin) {
  auto &artemis_pkg = pmb->packages.Get("artemis");
  Params &params = artemis_pkg->AllParams();
  if (!(params.hasKey("whistler_wave_params"))) {
    WHISTLER_WAVE_Params whistler_wave_params;
    // Left and right region
    whistler_wave_params.rho0 = pin->GetOrAddReal("problem", "rho0", 1.666666666667e-11);
    whistler_wave_params.b0 = pin->GetOrAddReal("problem", "b0", 0.1); // 1000G=0.1T
    whistler_wave_params.dB = pin->GetOrAddReal("problem", "dB", 0.001);
    whistler_wave_params.beta = pin->GetOrAddReal("problem", "beta", 1.e-4);
    whistler_wave_params.mode = pin->GetOrAddReal("problem", "mode", 1.);
    whistler_wave_params.Lx = pin->GetOrAddReal("problem", "Lx", 200.);
    params.Add("whistler_wave_params", whistler_wave_params);
  }
}

//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator::WHISTLER_WAVE()
//! \brief Sets initial conditions for whistler_wave problem
...
inline void ProblemGenerator(MeshBlock *pmb, ParameterInput *pin) {
  ...
  // WHISTLER_WAVE parameters
  auto shkp = artemis_pkg->Param<WHISTLER_WAVE_Params>("whistler_wave_params");
  ...
  // Setup whistler_wave state
  pmb->par_for(
      "whistler_wave", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        geometry::Coords<GEOM> coords(pco, k, j, i);
        const auto &xi = coords.GetCellCenter();
	const auto &dx = coords.GetCellWidths();
	const Real left_right = xi[0]>=0. ? 1. : -1.;
	const Real rho = shkp.rho0;                              
	const Real P = Pfac;	
	...
	v(0, gas::prim::Pe(0), k, j, i) = (Z_ion/(1+Z_ion))*P;
      });
  ...
  pmb->par_for(
	...
	vmag(0, TE::F1, gas::face::bfield(0), k, j, i) = B0norm; 
      });
  ...
  pmb->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        vmag(0, TE::F2, gas::face::bfield(0), k, j, i) = deltaB*std::sin(2.*M_PI*shkp.mode*xf[0]/Lx);
      });
  ...
  pmb->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        vmag(0, TE::F3, gas::face::bfield(0), k, j, i) = deltaB*std::cos(2.*M_PI*shkp.mode*xf[0]/Lx);
      });
