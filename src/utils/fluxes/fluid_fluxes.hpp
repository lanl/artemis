...
TaskStatus CalculateFluxes(MeshData<Real> *md, PKG &pkg, PackPrim vprim, PackFlux vflux,
  ...
  } else if (sys == Coordinates::spherical) {
    return CalculateFluxesRiemannSelect<Coordinates::spherical, FLUID_TYPE>(
        md, pkg, vprim, vflux, vface, pcm);
  ...
  parthenon::par_for(
      ...
      KOKKOS_LAMBDA(const int &b, const int &k, const int &j, const int &i) {
        ...
        for (int n = 0; n < nspecies; ++n) {
          ...
          if (x1dep) {
            vcons(b, IMX, k, j, i) += rdt * (dhdx1[0] * SQR(vprim(b, IVX, k, j, i)) +
                                             dhdx1[1] * SQR(vprim(b, IVY, k, j, i)) +
                                             dhdx1[2] * SQR(vprim(b, IVZ, k, j, i)));
          }
          if (x2dep) {
            vcons(b, IMY, k, j, i) += rdt * (dhdx2[0] * SQR(vprim(b, IVX, k, j, i)) +
                                             dhdx2[1] * SQR(vprim(b, IVY, k, j, i)) +
                                             dhdx2[2] * SQR(vprim(b, IVZ, k, j, i)));
          }
          if (x3dep) {
            vcons(b, IMZ, k, j, i) += rdt * (dhdx3[0] * SQR(vprim(b, IVX, k, j, i)) +
                                             dhdx3[1] * SQR(vprim(b, IVY, k, j, i)) +
                                             dhdx3[2] * SQR(vprim(b, IVZ, k, j, i)));
          }
        }
      });
...
TaskStatus FluxSourceGeomSelect(MeshData<Real> *md, PKG &pkg, PackPrim vprim,
  ...
  if (sys == C::cartesian) {
    return FluxSourceImpl<C::cartesian, FLUID_TYPE>(md, pkg, vprim, vcons, vface, dt);
  } else if (sys == C::spherical) {
    return FluxSourceImpl<C::spherical, FLUID_TYPE>(md, pkg, vprim, vcons, vface, dt);
  } else if (sys == C::cylindrical) {
    return FluxSourceImpl<C::cylindrical, FLUID_TYPE>(md, pkg, vprim, vcons, vface, dt);
  } else if (sys == C::axisymmetric) {
    return FluxSourceImpl<C::axisymmetric, FLUID_TYPE>(md, pkg, vprim, vcons, vface, dt);
  } else {
    PARTHENON_FAIL("Coordinate type not recognized!");
  }
}
