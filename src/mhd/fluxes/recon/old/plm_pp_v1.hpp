...
KOKKOS_INLINE_FUNCTION void pp_limiter(const int b, const int k, const int j, const int i, 
  ...
  const Real rhoe = (p + (dt/dx)*(neg_comp(vx*Dp) + gamma*p*neg_comp(Du))) 
	  	  / (gamma-1.);
  const Real fdW = rhoe>0. ? std::sqrt(rhoe/std::max(LdW, rhoe)) : 0.;
  if (std::isnan(fdW) or fdW<0. or fdW>1.) 
    printf("i=%d: fdW=%.4e, LdW=%.4e, rhoe=%.4e, DeltaPe=%.4e, Drho=%.4e, Dp=%.4e, DPe=%.4e, ne=%.4e \n", i,fdW,LdW,rhoe,DeltaPe,Drho,Dp,DPe,ne);
  // YH: Only apply to ideal MHD component! Rmb Pe shld have minimal diffusion!
