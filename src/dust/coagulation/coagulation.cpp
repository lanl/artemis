...
std::shared_ptr<StateDescriptor> Initialize(ParameterInput *pin) {
  ...
  if (std::exp(cond) > std::sqrt(2.0)) {
	...
	<< std::log(mmax / mmin) / (std::log(std::sqrt(2.0))) + 1.
  ...
  const int n1DRv = coag1DRv::last1;
  cpars.coagR2D = ParArray2D<Real>("coagReal2D", n1DRv, nm);
  ...
  initializeArray(nm, cpars.pGrid, cpars.rho_p, cpars.chi, a, dust_size,
		  cpars.klf, cpars.coagR2D, cpars.coagR3D,
		  cpars.cpod_notzero, cpars.cpod_short, dfloor);
  
  params.Add("coag_pars", cpars);

  // other parameters for coagulation
  int nstep1Coag = pin->GetOrAddReal("problem", "nstep1Coag", 50);
...
void initializeArray(const int nm, int &pGrid, const Real &rho_p, const Real &chi,
  ...
  auto mass_grid = Kokkos::subview(coag2D, imassg, Kokkos::ALL);
  int ifloord = coag1DRv::floor_dnum;
  auto floor_dnum = Kokkos::subview(coag2D, ifloord, Kokkos::ALL);
  parthenon::par_for(
...
} //end of namespace coagulation
} //end of namesapce dust
